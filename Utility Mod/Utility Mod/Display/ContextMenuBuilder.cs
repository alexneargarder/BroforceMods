...
namespace Utility_Mod
{
    ...
    public class ContextMenuBuilder
    {
        ...
        private readonly ContextMenuManager manager;
        ...
        {
            // Test Dummy submenu
            MenuItem testDummyMenu = new MenuItem( "Test Dummy" );
            BuildTestDummySubmenu( testDummyMenu );
            parentMenu.AddSubItem( testDummyMenu );
        ...
        public void BuildTestDummySubmenu( MenuItem parentMenu )
        {
            var spawnTestDummy = MenuAction.CreateSpawnTestDummy();
            parentMenu.AddSubItem( new MenuItem( "Spawn Test Dummy", () =>
            {
                manager.ExecuteAction( spawnTestDummy );
            } )
            {
                ActionId = spawnTestDummy.Id,
                MenuAction = spawnTestDummy
            } );

            parentMenu.AddSubItem( new MenuItem( "Clear All Test Dummies", () =>
            {
                TestDummyManager.RemoveAllDummies();

                // Track the clear action if recording
                if ( Main.settings.isRecordingLevelEdits )
                {
                    var editAction = LevelEditAction.CreateClearTestDummies();
            ...
            parentMenu.AddSubItem( new MenuItem( "Reset All Dummies", () =>
            {
                TestDummyManager.ResetAllDummies();
                manager.CloseMenu();
            } ) );

            parentMenu.AddSeparator();

            parentMenu.AddSubItem( new MenuItem( "Show DPS Overlay", (Action)null )
            {
                ShowCheckbox = true,
                IsToggleAction = true,
                IsChecked = Main.settings.showTestDummyDPS ?? true,
                OnToggle = ( isChecked ) =>
                {
                    Main.settings.showTestDummyDPS = isChecked;
                    TestDummyManager.SetDPSOverlayVisibility( isChecked );
                }
            } );
        }
